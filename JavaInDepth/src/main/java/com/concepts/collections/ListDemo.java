...
public class ListDemo {
	...
	private static List<Integer> arrayListDemo() {
		 ...
		 * list1.remove(Integer.valueOf(element)); // valueOf is used due to its
		 * caching } }
		 */
	...
	static void listIteratorDemo() {
		...
		for (ListIterator iterator = list.listIterator(); iterator.hasNext();) {
			...
			if (iterator.nextIndex() == 1) {
				...
				iterator.set("test");
				System.out
						.println("Setting element at index 1 as test1 to show that two set operations can be invoked sequentially");
				iterator.set("test1");
			}
		}
